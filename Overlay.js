// Copy of dojox/mobile/Overlay, not refactored to work with delite [yet]
define([
	"dojo/_base/declare",
	"dojo/_base/lang",
	"dojo/sniff",
	...
	return declare("dui.mobile.Overlay", Widget, {
		...
		_reposition: function () {
			...
			if ((popupPos.y + popupPos.h) !== vp.h // TODO: should be a has() test for position:fixed not scrolling
				// android 2.x supports position:fixed but child transforms don't persist
				|| (domStyle.get(this.domNode, "position") !== "absolute" && has("android") < 3)) {
		...
		show: function (/*DomNode?*/aroundNode) {
			...
			domClass.replace(_domNode, ["duiCoverv", "duiIn"],
				["duiOverlayHidden", "duiRevealv", "duiOut", "duiReverse", "duiTransition"]);
			this.defer(function () {
		...
		hide: function () {
			...
			if (has("css3-animations")) {
				domClass.replace(_domNode, ["duiRevealv", "duiOut", "duiReverse"],
					["duiCoverv", "duiIn", "duiOverlayHidden", "duiTransition"]);
				this.defer(function () {
					var handler = this.own(on(_domNode, css3.name("transitionEnd"), function () {
						handler.remove();
						domClass.replace(_domNode, ["duiOverlayHidden"],
				...
				domClass.replace(_domNode, ["duiOverlayHidden"],
					["duiCoverv", "duiIn", "duiRevealv", "duiOut", "duiReverse"]);
			}
		},
