// Copy of dojox/mobile/Tooltip, not refactored to work with delite [yet]
...
define([
	...
	var Tooltip = declare(has("dojo-bidi") ? "dui.mobile.NonBidiTooltip" : "dui.mobile.Tooltip", Widget, {
		...
		show: function (/*DomNode*/ aroundNode, /*Array*/positions) {
			...
			var best = place.around(domNode, aroundNode, positions ||
				["below-centered", "above-centered", "after-centered", "before-centered"], this.isLeftToRight());
			...
			domStyle.set(this.anchor, (connectorClass === "duiTooltipAbove" || connectorClass === "duiTooltipBelow")
					? { top: "", left: Math.max(0, pos.x - best.x + (pos.w >> 1) -
						(this.arrow.offsetWidth >> 1)) + "px" }
					: { left: "", top: Math.max(0, pos.y - best.y + (pos.h >> 1) -
						(this.arrow.offsetHeight >> 1)) + "px" }
			);
