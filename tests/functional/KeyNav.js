define([
	...
	registerSuite({
		...
		"nested focusable nodes": {
			beforeEach: function () {
				return this.remote.findById("nested_before").click().end();
			},

			"click <button> in header": function () {
				return this.remote
					.execute("return document.getElementById('nested').getAttribute('tabindex');").then(function (ti) {
						assert.equal(ti, "0", "container tabindex starts at 0");
					})
					.execute("return document.getElementById('header-one').getAttribute('tabindex');")
					.then(function (ti) {
						assert.equal(ti, "-1", "header tabindex starts at -1");
					})
					.findById("header-one-button").click().end()
					.execute("return document.activeElement.id;").then(function (text) {
						assert.strictEqual(text, "header-one-button");
					})
					.execute("return document.getElementById('nested').hasAttribute('tabindex');").then(function (b) {
						assert.isFalse(b, "tabindex removed from container");
					})
					.execute("return document.getElementById('header-one').hasAttribute('tabindex');")
							.then(function (b) {
						assert.isFalse(b, "tabindex removed from header");
					})
					.execute("return document.getElementById('header-one').className;").then(function (c) {
						assert.strictEqual(c, "d-active-descendant", "header class");
					})
					.findById("nested_before").click().end()
					.execute("return document.getElementById('nested').getAttribute('tabindex');").then(function (ti) {
						assert.equal(ti, "0", "container tabindex set back to 0");
					})
					.execute("return document.getElementById('header-one').getAttribute('tabindex');")
							.then(function (ti) {
						assert.equal(ti, "-1", "header tabindex set back to -1");
					})
					.execute("return document.getElementById('header-one').className;").then(function (c) {
						assert.strictEqual(c, "", "d-active-descendant removed from header class");
					});
			},

			"focus header then click <button> in another header": function () {
				if (this.remote.environmentType.brokenSendKeys || !this.remote.environmentType.nativeEvents) {
					return this.skip("no keyboard support");
				}
				...
				return this.remote.pressKeys(keys.TAB)
					.execute("return document.activeElement.id;").then(function (id) {
						assert.strictEqual(id, "header-one", "tabbing into container goes to first header");
					})
					.execute("return document.getElementById('header-one').className;").then(function (c) {
						assert.strictEqual(c, "d-active-descendant", "header class");
					})
					.findById("header-two-button").click().end()
					.execute("return document.activeElement.id;").then(function (text) {
						assert.strictEqual(text, "header-two-button", "make sure header one doesn't steal focus");
					})
					.execute("return document.getElementById('nested').hasAttribute('tabindex');").then(function (b) {
						assert.isFalse(b, "tabindex removed from container");
					})
			...
			"focus header then click <button> in panel": function () {
				if (this.remote.environmentType.brokenSendKeys || !this.remote.environmentType.nativeEvents) {
					return this.skip("no keyboard support");
				}
				...
				return this.remote.pressKeys(keys.TAB)
					...
					.findById("panel-two-button").click().end()
					.execute("return document.activeElement.id;").then(function (text) {
						assert.strictEqual(text, "panel-two-button", "make sure header one doesn't steal focus");
					})
					.execute("return document.getElementById('nested').hasAttribute('tabindex');").then(function (b) {
						assert.isFalse(b, "tabindex removed from container");
					})
