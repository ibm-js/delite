define([], function () {
	...
	registerSuite("HasDropDown functional tests", {
		before: function () {
			return this.remote
				.get("delite/tests/functional/HasDropDown.html")
				...
				mouse: function () {
					...
					return this.remote
						...
						.execute(function () {
							...
							// Exception: currently, on mobile dropdown is always focused,
							// see HasDropDown#openDropDown().
				...
				return this.remote
					...
					.then(function (ret) {
						...
						assert(Math.abs(viewport.w / 2 - popupCoords.left - popupCoords.width / 2) < 1,
							"centered horizontally, " + viewport.w + ", " +
							popupCoords.width + ", " + popupCoords.left);
					})
