define([], function () {
	...
	var tests = {
		"repeat move off screen": function () {
			...
			// No actual assert here, we're relying on the browser throwing an exception to webdriver.
			return this.remote
		...
		"popup-after-position event": {
			before: function () {
				return this.remote
					.execute(function () { require("delite/scrollIntoView")(tooltipDropDownButton) })
	...
	registerSuite("popup functional tests", {
		"popup attached to body": {
			before: function () {
				return this.remote
					.get("delite/tests/functional/popup.html?sibling=false")
