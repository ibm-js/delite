define([
	...
	var tests = {
		at: {
			atTL: function () {
				...
				assert.strictEqual(popupCoords.left, 10, "x coord");
				assert.strictEqual(popupCoords.top, 7, "y coord, page scroll is " + window.pageYOffset);
			...
			atTR: function () {
				...
				assert.strictEqual(popupCoords.top, 7, "y coord, window scroll is " + window.pageYOffset);
				assert.strictEqual(ret.w, 75, "it's 75px wide");
			}
		},
		...
		around: {
			...
			tests: {
				aroundT: function () {
					...
					assert.strictEqual(ret.aroundCorner, "BL",
						"around corner, window.pageYOffset = " + window.pageYOffset);
					assert.strictEqual(ret.corner, "TL", "popup's corner");
					assert.strictEqual(Math.round(popupCoords.top), 20, "underneath around node");
					assert.strictEqual(Math.round(popupCoords.left),
				...
				aroundB: function () {
					...
					var ret = place.around(popup, aroundBottom, [
						"below",	// aroundBottom's bottom-left corner with popup's top-left corner (fails)
						"above",	// aroundBottom's top-left corner with popup's bottom-left corner (works)
						"below-alt"	// aroundBottom's bottom-right corner with popup's top-right corner (fails)
					], true);
				...
				aroundMR: function () {
					...
					var aroundPos = aroundRight.getBoundingClientRect(),
						popupPos = popup.getBoundingClientRect();
					assert.strictEqual(Math.round(aroundPos.left), Math.round(popupPos.right),
						"before around node");
					assert(aroundPos.top > popupPos.top, "starts before around node");
					assert(aroundPos.top < popupPos.bottom, "ends after around node");
				},
	...
	registerSuite("place", {
		...
		"popup attached to sub node": {
			before: function () {
				wrapper = document.createElement("div");
				wrapper.style.cssText = "position: absolute; top: 100px; left: 100px";
