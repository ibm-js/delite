	...
	registerSuite("KeyNav", {
		...
		tests: {
			getNext: function () {
				...
				assert.strictEqual(scanAll(1).join(", "),
					"child 1, child 2, child 3, child 4, child 5, child 6, child 1", "children in natural order");

				assert.strictEqual(scanAll(-1).join(", "),
					"child 6, child 5, child 4, child 3, child 2, child 1, child 6", "children in reverse order");
			},
