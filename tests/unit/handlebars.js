	...
	registerSuite("handlebars", {
		...
		tests: {
			...
			"special characters": function () {
				...
				assert.strictEqual(myList.firstChild.getAttribute("bar"), "\\\"hello\"",
					"double quotes, backslash prop");
			...
			"special characters 2": function () {
				...
				assert.strictEqual(myJson.children[0].getAttribute("attr"), "{name: {first: 'john', last: 'doe'}}",
					"json");
			...
			"widgets in templates": {
				...
				*/
				...
				"attribute boolean/number literal": function () {
					// This widget uses sub-widgets handlebars-button (defined in first test)
					// and also handlebars-heading.
			...
			"whitespace": function () {
				...
				ws.deliver();
				assert.strictEqual(ws.childNodes.length, 3,
					"middle whitespace preserved, start/end whitespace deleted");
